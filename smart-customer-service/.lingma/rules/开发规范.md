...
```python
from utils.data_masking import sanitize_for_logging

# 记录日志前必须脱敏
logger.info(f"处理请求: {sanitize_for_logging(request_data)}")

# 禁止直接记录包含敏感信息的字典
logger.debug(f"用户数据: {request_data}")  # ❌ 错误
```
...
- ✅ **始终**对用户 ID 使用哈希而非明文
- ✅ **始终**使用环境变量存储 API 密钥
- ✅ **始终**在记录日志前调用 `sanitize_for_logging()`
- ❌ **禁止**记录密码或 API 密钥
- ❌ **禁止**在日志中记录完整的请求/响应体
- ❌ **禁止**将敏感数据写入文件或数据库明文存储
...
class SendMessageRequest(BaseModel):
    ...
    def validate_conversation_id(cls, v):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', v):
            raise ValueError('会话ID只能包含字母、数字、下划线和连字符')
        return v
    ...
    def validate_content(cls, v):
        ...
        # 限制特殊字符数量,防止注入攻击
        if v.count('<') > 10 or v.count('>') > 10:
            raise ValueError('消息内容包含过多HTML标签')
        return v.strip()
```
...
async def post_message(request: Request, conversation_id: str, data: SendMessageRequest):
    # 处理消息
    pass
...
# 在需要认证的路由中使用
@app.post("/api/webhook/escalation")
async def webhook_escalation(payload: dict, authenticated: bool = Depends(verify_api_key)):
    # 只有验证通过的请求才能执行
...
- ✅ **始终**对 CORS 配置明确的域名白名单
- ✅ **始终**验证和清理所有用户输入
- ✅ **始终**使用参数化查询防止 SQL 注入
- ✅ **始终**对用户输出进行 HTML 转义防止 XSS
- ✅ **始终**对 API 端点实施速率限制
- ✅ **始终**验证 API 密钥和身份凭证
- ✅ **始终**限制请求体大小
- ✅ **始终**添加安全响应头
