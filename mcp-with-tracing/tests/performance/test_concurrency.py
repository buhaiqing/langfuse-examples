...
class TestThreadSafety:
    ...
    def test_concurrent_session_creation(self):
        """测试并发会话创建的线程安全性"""
        from src.observability.session import clear_session, get_session_id, set_session
        ...
        def create_and_verify_session(session_num):
            try:
                ...
                with lock:
                    errors.append(f"Session {session_num}: {e!s}")
    ...
    def test_concurrent_feedback_submission(self):
        ...
        def submit_feedback(feedback_num):
            try:
                ...
                with lock:
                    errors.append(f"Feedback {feedback_num}: {e!s}")
    ...
    def test_concurrent_alert_checking(self):
        ...
        def check_alert(check_num):
            try:
                ...
                with lock:
                    errors.append(f"Check {check_num}: {e!s}")
    ...
    def test_smart_alert_manager_under_load(self):
        ...
        def run_detection_cycle(cycle_num):
            try:
                ...
                with lock:
                    errors.append(f"Cycle {cycle_num}: {e!s}")
