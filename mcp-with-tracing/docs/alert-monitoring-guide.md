...
**检测间隔**: 默认 5 分钟 (`ALERT_CHECK_INTERVAL_MINUTES`)
...
**核心功能**:
1. ✅ **自动模型训练** - 从历史数据学习正常模式
2. ✅ **单变量异常检测** - Prophet 时间序列预测
3. ✅ **多变量异常检测** - PyOD 多维分析
4. ✅ **自适应阈值** - 无需手动配置

**检测间隔**: 默认 10 分钟 (`SMART_ALERT_CHECK_INTERVAL_MINUTES`)

**依赖**: `prophet`, `pyod`, `pandas`, `numpy`, `scikit-learn`
...
| **依赖** | 无额外依赖 | ML 库 (prophet, pyod等) |

**推荐策略**:
- ✅ **同时启用**: 两者互补，不冲突
- ✅ **传统告警**: 处理明确的业务规则（如成功率 < 95%）
- ✅ **智能告警**: 发现潜在问题（如成功率突然下降 20%，即使仍 > 95%）
- ✅ **共享通知**: 都使用相同的 AlertChannel 和 Notifier
...
└──────────────┬──────────────────────────────┘
               ...
               ▼
┌─────────────────────────────────────────────┐
│  Start SmartAlertManager                    │
│  (Background Monitoring Thread)             │
└──────────────┬──────────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────────┐
│  Detection Cycle (every N minutes):         │
│  ┌───────────────────────────────────────┐  │
│  │ 1. Collect Metrics from Langfuse      │  │
